...
class RoleController extends Controller
{
    ...
    public function upsertrole(Request $request)
    {
        ...
        if(trim($name) == 'Super Admin' || trim($name) == 'super admin') return Redirect('/admin/role')->with('errorMessage', 'You are not allowed to create role Super Admin');
        if ($id == '') {
            $role = [
                'name' => $name,
